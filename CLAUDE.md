...
Photos carry **tags**, modelled like **GitHub labels**: a `tags` catalog table (`name` unique + `color`) with `routes/tags.js` (channel `tags`, CRUD + `ensureTags()` helper). A photo still stores its tags as a JSON array of **names** in `photos.tags`; saving a photo auto-registers any unknown name in the catalog with a stable color, and `POST /api/tags` does the same on demand. Colors come from `src/api/tagColors.js` — an **isomorphic** module (imported by both the client and `routes/tags.js`, so it must stay dependency-free; the Dockerfile copies it alongside `roomCatalog.js`): `colorForName()` derives a stable palette color from the name (FNV-1a hash), `tagTextColor()` picks black/white text for contrast. Renaming/deleting a tag propagates to every photo's `tags` array. Client side, `api/useTags.js` loads the catalog (+`colorOf`/`ensure`), and `TagPicker.jsx` is the GitHub-style combobox (colored pill chips, dropdown with search, click-to-toggle existing tags, "create" row for new ones) used in `PhotoAttachModal` and the photo cards. `PhotosView` has a colored, clickable tag-filter bar that AND-matches selected tags, plus a free-text search over captions and tag names. **Drag & drop** (native HTML5, no library): dropping OS image files onto `PhotosView` imports them, dropping onto a `PhotosPanel` imports **and** links in one gesture (`api/useFileDrop.js`); thumbnails are draggable to reorder — library order persists via `photos.sort_order` (`PUT /photos/order`), per-entity order via `links.sort_order` (`PUT /photos/entity-order`), both driven by `api/useDragReorder.js`. Library reorder works even while a tag/text filter is active: the visible photos are permuted among the global-order slots they occupy and filtered-out photos keep their positions (`persistOrder` in `PhotosView` merges the reordered visible subset back into the full list before `PUT /photos/order`). `PhotosFor`/list queries order by `sort_order ASC, created_at DESC`, so new uploads (sort_order 0) appear first until explicitly reordered.

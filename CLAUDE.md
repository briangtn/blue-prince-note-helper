...
Photos carry **tags** (`tags` column, JSON array; `TagInput` component for chip editing; `PhotosView` has a tag-filter bar that AND-matches selected tags). **Drag & drop** (native HTML5, no library): dropping OS image files onto `PhotosView` imports them, dropping onto a `PhotosPanel` imports **and** links in one gesture (`api/useFileDrop.js`); thumbnails are draggable to reorder — library order persists via `photos.sort_order` (`PUT /photos/order`), per-entity order via `links.sort_order` (`PUT /photos/entity-order`), both driven by `api/useDragReorder.js`. Library reorder is disabled while a tag filter is active (order is global). `PhotosFor`/list queries order by `sort_order ASC, created_at DESC`, so new uploads (sort_order 0) appear first until explicitly reordered.
